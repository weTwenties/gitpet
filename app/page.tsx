...
export default function Home() {
  const [username, setUsername] = useState("khanhvinhnguyen");
  const [user, setUser] = useState<GitHubUser>(demoUser);
  const [pet, setPet] = useState<PetProfile>(demoPet);
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState("Demo profile — search any public GitHub username");

  async function summonPet(event: FormEvent<HTMLFormElement>) {
